  ...
  def index
    ...
    if (current_user.role.is_dinkes_region) || (current_user.role.is_public_health_center) || (current_user.role.is_surveilance)
      if (current_user.role.is_dinkes_region)
        @telegram_message_closecont_reporters = @search.result(distinct: true).includes(:username_reporter).where("telegram_username_reporters.main_city_id = ?", current_user.dinkes_region.city.id).newest_first.page params[:page]
      else
        ...
        @telegram_message_closecont_reporters = @search.result(distinct: true).includes(:username_reporter).where("telegram_username_reporters.main_sub_district_id = ?", sub_districts).newest_first.page params[:page]
      end
    else
