  ...
  def index
    ...
    if (current_user.role.is_dinkes_region) || (current_user.role.is_public_health_center) || (current_user.role.is_surveilance)
      if (current_user.role.is_dinkes_region)
        ...
        @telegram_username_reporters = @search.result(distinct: true).where(main_public_health_center_id: current_user.public_health_center.id).newest_first.page params[:page]
      end
    else
