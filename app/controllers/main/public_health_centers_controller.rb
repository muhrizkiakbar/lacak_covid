class Main::PublicHealthCentersController < ApplicationController
  ...
  def index
    @search = Main::PublicHealthCenter.ransack(params[:q])
    @main_public_health_centers = @search.result(distinct: true).where(main_dinkes_region_id: @main_dinkes_region.id).page params[:page]
