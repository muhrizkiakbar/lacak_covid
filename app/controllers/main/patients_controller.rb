class Main::PatientsController < ApplicationController
  ...
  private
    ...
    def set_main_patient
      @main_patient = Main::Patient.find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def main_patient_params
      params.require(:main_patient).permit(:main_tribe_id, :main_city_id, :main_district_id, :main_sub_district_id, :main_citizen_association_id, :main_neighborhood_association_id, :main_marital_status_id, :no_identity, :name_of_parent, :name, :address, :phone_number, :date_of_birth, :gender)
    end
end
